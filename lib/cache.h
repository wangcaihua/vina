...
struct cache : public igrid {
  ...
  fl eval_deriv(model& m,
                fl v) const;  // needs m.coords, sets m.minus_forces // clean up
#if 0                         // no longer doing I/O of the cache
	void read(const path& name); // can throw cache_mismatch
	void write(const path& name) const;
#endif
  void populate(const model& m, const precalculate& p,
                const szv& atom_types_needed, bool display_progress = true);
