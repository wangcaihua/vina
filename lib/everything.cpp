...
inline fl gaussian(fl x, fl width) { return std::exp(-sqr(x / width)); }
...
struct electrostatic : public distance_additive {
  ...
  electrostatic(fl cap_, fl cutoff_) : distance_additive(cutoff_), cap(cap_) {
    name = std::string("electrostatic(i=") + to_string(i) +
           ", ^=" + to_string(cap) + ", c=" + to_string(cutoff) + ")";
  }
...
struct ad4_solvation : public distance_additive {
  ...
  ad4_solvation(fl desolvation_sigma_, fl solvation_q_, bool charge_dependent_,
                fl cutoff_)
      : distance_additive(cutoff_),
        solvation_q(solvation_q_),
        charge_dependent(charge_dependent_),
        desolvation_sigma(desolvation_sigma_) {
    name = std::string("ad4_solvation(d-sigma=") +
           to_string(desolvation_sigma) + ", s/q=" + to_string(solvation_q) +
           ", q=" + to_string(charge_dependent) + ", c=" + to_string(cutoff) +
           ")";
  }
  fl eval(const atom_base& a, const atom_base& b, fl r) const {
    fl q1 = a.charge;
...
struct gauss : public usable {
  ...
  gauss(fl offset_, fl width_, fl cutoff_)
    ...
    name = std::string("gauss(o=") + to_string(offset) +
           ", w=" + to_string(width) + ", c=" + to_string(cutoff) + ")";
  }
...
struct hydrophobic : public usable {
  ...
  hydrophobic(fl good_, fl bad_, fl cutoff_)
    ...
    name = "hydrophobic(g=" + to_string(good) + ", b=" + to_string(bad) +
           ", c=" + to_string(cutoff) + ")";
  }
  fl eval(sz t1, sz t2, fl r) const {
...
struct non_hydrophobic : public usable {
  ...
  non_hydrophobic(fl good_, fl bad_, fl cutoff_)
      : usable(cutoff_), good(good_), bad(bad_) {
    name = "non_hydrophobic(g=" + to_string(good) + ", b=" + to_string(bad) +
           ", c=" + to_string(cutoff) + ")";
  }
  fl eval(sz t1, sz t2, fl r) const {
    if (!xs_is_hydrophobic(t1) && !xs_is_hydrophobic(t2))
...
struct vdw : public usable {
  ...
  vdw(fl smoothing_, fl cap_, fl cutoff_)
    ...
    name = "vdw(i=" + to_string(i) + ", j=" + to_string(j) +
           ", s=" + to_string(smoothing) + ", ^=" + to_string(cap) +
           ", c=" + to_string(cutoff) + ")";
  }
  fl eval(sz t1, sz t2, fl r) const {
...
struct non_dir_h_bond : public usable {
  ...
  non_dir_h_bond(fl good_, fl bad_, fl cutoff_)
      : usable(cutoff_), good(good_), bad(bad_) {
    name = std::string("non_dir_h_bond(g=") + to_string(good) +
           ", b=" + to_string(bad) + ")";
  }
  fl eval(sz t1, sz t2, fl r) const {
...
fl smooth_div(fl x, fl y) {
  ...
  if (std::abs(y) < epsilon_fl)
    return ((x * y > 0) ? max_fl
                        : -max_fl);  // FIXME I hope -max_fl does not become NaN
  return x / y;
}
...
everything::everything() {  // enabled according to design.out227
  ...
  // FIXME? enable some?
  //// distance_additive
  // add(d, new ad4_solvation(3.6, 0.01097,  true, cutoff)); //
  // desolvation_sigma, solvation_q, charge_dependent, cutoff add(d, new
  // ad4_solvation(3.6, 0.01097, false, cutoff)); // desolvation_sigma,
  // solvation_q, charge_dependent, cutoff
  ...
  add(1,
      new gauss(0, 0.5, cutoff));  // offset, width, cutoff // WEIGHT: -0.035579
  ...
  add(1,
      new gauss(3, 2.0, cutoff));  // offset, width, cutoff // WEIGHT: -0.005156
  ...
  add(1, new hydrophobic(0.5, 1.5,
                         cutoff));  // good, bad, cutoff // WEIGHT:  -0.035069
  ...
  add(1, new non_dir_h_bond(-0.7, 0,
                            cutoff));  // good, bad, cutoff // WEIGHT: -0.587439
