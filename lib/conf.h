...
inline void torsions_increment(flv& torsions, const flv& c,
                               fl factor) {  // new torsions are normalized
  VINA_FOR_IN(i, torsions) {
...
inline void torsions_generate(flv& torsions, fl spread, fl rp, const flv* rs,
                              rng& generator) {
  assert(!rs || rs->size() == torsions.size());  // if present, rs should be the
                                                 // same size as torsions
  VINA_FOR_IN(i, torsions)
...
struct rigid_conf {
  ...
  void increment(const rigid_change& c, fl factor) {
    ...
    quaternion_increment(
        orientation, rotation);  // orientation does not get normalized; tests
                                 // show rounding errors growing very slowly
  }
...
struct residue_change {
  flv torsions;
  void print() const { printnl(torsions); }
};

struct residue_conf {
  flv torsions;
  void set_to_null() { torsions_set_to_null(torsions); }
  void increment(const residue_change& c, fl factor) {
    torsions_increment(torsions, c.torsions, factor);
  }
  void randomize(rng& generator) { torsions_randomize(torsions, generator); }
  void print() const { printnl(torsions); }
...
struct change {
  ...
  fl& operator()(sz index) {
    ...
    VINA_CHECK(false);
    return ligands[0]
        .rigid.position[0];  // shouldn't happen, placating the compiler
  }
  sz num_floats() const {
    sz tmp = 0;
    VINA_FOR_IN(i, ligands)
...
struct conf {
  ...
  void increment(const change& c,
                 fl factor) {  // torsions get normalized, orientations do not
    VINA_FOR_IN(i, ligands)
  ...
  bool too_close(const conf& c, const scale& cutoff) const {
    return internal_too_close(c, cutoff.torsion) &&
           external_too_close(c, cutoff);  // a more efficient implementation is
                                           // possible, probably
  }
  void generate_internal(
      fl torsion_spread, fl rp, const conf* rs,
  ...
  void generate_external(
      const scale& spread, fl rp, const conf* rs,
      rng& generator) {  // torsions are not normalized after this
    VINA_FOR_IN(i, ligands) {
...
inline bool operator<(const output_type& a,
                      const output_type& b) {  // for sorting output_container
  return a.e < b.e;
}
