...
struct usable : public distance_additive {
  ...
  usable(fl cutoff_)
      : distance_additive(cutoff_), atom_typing_used(atom_type::XS) {}
  fl eval(const atom_base& a, const atom_base& b,
          fl r) const {  // should not be overriden
...
struct conf_independent_inputs {
 ...
 private:
  unsigned num_bonded_heavy_atoms(const model& m, const atom_index& i)
      const;  // FIXME? - could be static, but I don't feel like declaring
              // function friends
...
struct term_set {
  ...
  void get_names(bool enabled_only,
                 std::vector<std::string>& out) const {  // appends to "out"
    VINA_CHECK(enabled.size() == fun.size());
    VINA_FOR_IN(i, fun)
    if (!enabled_only || enabled[i]) out.push_back(fun[i].name);
  }
  void filter(flv::const_iterator& in, flv& out) const {
    VINA_CHECK(enabled.size() == fun.size());
    VINA_FOR_IN(i, enabled) {
      if (enabled[i]) out.push_back(*in);
      ++in;
    }
  }
  fl max_cutoff() const {
...
struct factors {
  ...
  // sz num_weights() const { return (std::max)(e.size(), i.size()); } // FIXME?
  // compiler bug? getting warnings here
