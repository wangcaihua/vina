...
struct parsing_struct {
  ...
  boost::optional<atom_reference>
      axis_begin;  // the index (in non_rigid_parsed::atoms) of the parent bound
                   // to immobile atom (if already known)
  boost::optional<atom_reference>
      axis_end;  // if immobile atom has been pushed into
...
model parse_receptor_pdbqt(const path& rigid_name,
                           const path& flex_name) {  // can throw parse_error
  rigid r;
  non_rigid_parsed nrp;
  context c;
