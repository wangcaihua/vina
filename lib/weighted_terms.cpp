...
weighted_terms::weighted_terms(const terms* t, const flv& weights)
    : t(t), weights(weights), cutoff_(0) {  // does not own t
...
fl weighted_terms::eval(sz t1, sz t2,
                        fl r) const {  // intentionally not checking for cutoff
  fl acc = 0;
...
fl weighted_terms::conf_independent(const model& m, fl e) const {
  ...
  conf_independent_inputs in(m);  // FIXME quite inefficient, but I think speed
