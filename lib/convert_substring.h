...
inline bool substring_is_blank(
    const std::string& str, sz i,
    sz j) {  // indexes are 1-based, the substring should be non-null
  if (i < 1 || i > j + 1 || j > str.size()) throw bad_conversion();
  VINA_RANGE(k, i - 1, j)
...
// when this was written, lexical cast to unsigned didn't work very well with
// "-123", etc.
template <>
inline unsigned convert_substring<unsigned>(
    const std::string& str, sz i,
    sz j) {  // indexes are 1-based, the substring should be non-null
