...
// then covalent radius added from
...
const fl metal_covalent_radius =
    1.75;  // for metals not on the list // FIXME this info should be moved to
           // non_ad_metals
...
const sz atom_equivalences_size =
    sizeof(atom_equivalence_data) / sizeof(const atom_equivalence);
...
const sz acceptor_kinds_size =
    sizeof(acceptor_kind_data) / sizeof(acceptor_kind);
...
inline bool ad_is_heteroatom(sz ad) {  // returns false for ad >= AD_TYPE_SIZE
  return ad != AD_TYPE_A && ad != AD_TYPE_C && ad != AD_TYPE_H &&
         ad != AD_TYPE_HD && ad < AD_TYPE_SIZE;
}
...
inline sz ad_type_to_el_type(sz t) {
  ...
  return EL_TYPE_SIZE;  // to placate the compiler in case of warnings - it
...
inline sz string_to_ad_type(
    const std::string&
        name) {  // returns AD_TYPE_SIZE if not found (no exceptions thrown,
                 // because metals unknown to AD4 are not exceptional)
