...
branch_metrics get_branch_metrics(const T& t) {
  ...
  if (!t.children.empty()) {
    ...
    VINA_FOR_IN(i, t.children) {
      ...
      lengths.push_back(res.length +
                        1);  // FIXME? weird compiler warning (sz -> unsigned)
    }
...
// FIXME hairy code - needs to be extensively commented, asserted, reviewed and
// tested
...
class appender {
  ...
  sz new_grid_index(sz x) const {
    return (is_a ? x
                 : (a_info.grid_atoms_size +
                    x));  // a-grid_atoms spliced before b-grid_atoms
  }
 ...
 public:
  ...
  sz operator()(sz x) const {  // transform coord index
    if (is_a) {
      if (x < a_info.m_num_movable_atoms)
        ...
        return x +
               b_info.m_num_movable_atoms;  // b-movable spliced before a-inflex
    } else {
      if (x < b_info.m_num_movable_atoms)
        return x +
               a_info
                   .m_num_movable_atoms;  // a-movable spliced before b-movable
      else
  ...
  void update(atom& a) const {
    VINA_FOR_IN(i, a.bonds) {
      bond& b = a.bonds[i];
      b.connected_atom_index = operator()(
          b.connected_atom_index);  // atom_index transformation, above
    }
  }
  ...
  void coords_append(
    ...
    std::vector<T> b_copy(
        b);  // more straightforward to make a copy of b and transform that than
             // to do piecewise transformations of the result
...
void model::append(const model& m) {
  ...
  t.coords_append(minus_forces,
                  m.minus_forces);  // for now, minus_forces.size() ==
                                    // coords.size() (includes inflex)
...
bool model::atom_exists_between(const distance_type_matrix& mobility,
    ...
    const {  // there is an atom closer to both a and b then they are to each
             // other and immobile relative to them
...
struct beads {
  ...
  beads(sz reserve_size, fl radius_sqr_) : radius_sqr(radius_sqr_) {
...
void model::assign_bonds(
    const distance_type_matrix&
        mobility) {  // assign bonds based on relative mobility, distance and
                     // covalent length
  ...
  VINA_FOR(i, n) {
    ...
    const fl max_covalent_r =
        max_covalent_radius();  // FIXME mv to atom_constants
    ...
    const fl bead_cutoff_sqr =
        sqr(bead_radius + bond_length_allowance_factor *
    ...
    VINA_FOR_IN(b, beads_instance.data) {
      ...
      VINA_FOR_IN(bead_elements_i, bead_elements) {
        ...
        distance_type dt =
            distance_type_between(mobility, i_atom_index, j_atom_index);
    ...
    switch (a.el) {
      ...
      case EL_TYPE_N:
        x = (acceptor && donor_NorO)
                ? XS_TYPE_N_DA
                : (acceptor ? XS_TYPE_N_A
                            : (donor_NorO ? XS_TYPE_N_D : XS_TYPE_N_P));
        break;
      case EL_TYPE_O:
        x = (acceptor && donor_NorO)
                ? XS_TYPE_O_DA
                : (acceptor ? XS_TYPE_O_A
                            : (donor_NorO ? XS_TYPE_O_D : XS_TYPE_O_P));
        break;
      case EL_TYPE_S:
        x = XS_TYPE_S_P;
        break;
...
conf model::get_initial_conf() const {  // torsions = 0, orientations =
                                        // identity, ligand positions = current
...
fl model::gyration_radius(sz ligand_number) const {
  ...
  VINA_RANGE(i, lig.begin, lig.end) {
    if (atoms[i].el != EL_TYPE_H) {  // only heavy atoms are used
      acc +=
          vec_distance_sqr(coords[i], lig.node.get_origin());  // FIXME? check!
      ++counter;
    }
  }
...
fl eval_interacting_pairs_deriv(const precalculate& p, fl v,
                                const interacting_pairs& pairs,
                                const vecv& coords,
                                vecv& forces) {  // adds to forces  // clean up
  const fl cutoff_sqr = p.cutoff_sqr();
  fl e = 0;
  VINA_FOR_IN(i, pairs) {
    const interacting_pair& ip = pairs[i];
...
fl model::evali(const precalculate& p, const vec& v) const {  // clean up
  fl e = 0;
  VINA_FOR_IN(i, ligands)
  e += eval_interacting_pairs(
      p, v[0], ligands[i].pairs,
      internal_coords);  // probably might was well use coords here
  return e;
}

fl model::evale(const precalculate& p, const igrid& ig,
                const vec& v) const {  // clean up
...
fl model::eval_deriv(const precalculate& p, const igrid& ig, const vec& v,
                     const conf& c, change& g) {  // clean up
  set(c);
...
fl model::rmsd_lower_bound_asymmetric(
    const model& x, const model& y) const {  // actually static
...
fl model::clash_penalty_aux(const interacting_pairs& pairs) const {
  ...
  VINA_FOR_IN(i, pairs) {
    ...
    const fl covalent_r =
        atoms[ip.a].covalent_radius() + atoms[ip.b].covalent_radius();
