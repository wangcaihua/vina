...
void grid::init(const grid_dims& gd) {
  ...
  assert(m_range[2] > 0);
  m_dim_fl_minus_1 =
      vec(m_data.dim0() - 1.0, m_data.dim1() - 1.0, m_data.dim2() - 1.0);
  VINA_FOR(i, 3) {
...
fl grid::evaluate_aux(const vec& location, fl slope, fl v,
  ...
  const fl penalty =
      slope * (miss * m_factor_inv);  // FIXME check that inv_factor is
                                      // correctly initialized and serialized
  ...
  if (deriv) {  // valid pointer
    const fl x_g = f000 * (-1) * my * mz + f100 * 1 * my * mz +
                   f010 * (-1) * y * mz + f110 * 1 * y * mz +
                   f001 * (-1) * my * z + f101 * 1 * my * z +
                   f011 * (-1) * y * z + f111 * 1 * y * z;

    const fl y_g = f000 * mx * (-1) * mz + f100 * x * (-1) * mz +
                   f010 * mx * 1 * mz + f110 * x * 1 * mz +
                   f001 * mx * (-1) * z + f101 * x * (-1) * z +
                   f011 * mx * 1 * z + f111 * x * 1 * z;

    const fl z_g = f000 * mx * my * (-1) + f100 * x * my * (-1) +
                   f010 * mx * y * (-1) + f110 * x * y * (-1) +
                   f001 * mx * my * 1 + f101 * x * my * 1 + f011 * mx * y * 1 +
                   f111 * x * y * 1;
