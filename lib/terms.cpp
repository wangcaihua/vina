...
unsigned conf_independent_inputs::num_bonded_heavy_atoms(
    const model& m,
    const atom_index& i) const {  // FIXME? - could be static, but I don't feel
                                  // like declaring function friends
  unsigned acc = 0;
...
unsigned conf_independent_inputs::atom_rotors(const model& m,
  ...
  VINA_FOR_IN(j, bonds) {
    ...
    if (b.rotatable && !a.is_hydrogen() &&
        num_bonded_heavy_atoms(m, b.connected_atom_index) >
            1)  // not counting CH_3, etc
      ++acc;
  }
  return acc;
}
...
std::vector<std::string> conf_independent_inputs::get_names()
    const {  // FIXME should probably be static
...
fl factors::eval(const flv& weights, bool include_internal) const {
  ...
  if (include_internal) tmp += inner_product_shortest(i, weights);
  return tmp;
}

// terms

std::vector<std::string> terms::get_names(
    bool enabled_only) const {  // does not include conf-independent
  std::vector<std::string> tmp;
...
sz terms::size_internal() const {
  return distance_additive_terms.size() + usable_terms.size() +
         additive_terms.size();
}

sz terms::size_conf_independent(
    bool enabled_only) const {  // number of parameters does not necessarily
                                // equal the number of operators
  sz acc = 0;
...
void terms::eval_additive_aux(const model& m, const atom_index& i,
                              const atom_index& j, fl r,
                              flv& out) const {  // out is added to
...
flv terms::evale(const model& m) const {
  ...
  if (brick_distance_sqr(box_begin, box_end, m.grid_atoms[j].coords) <
      max_r_cutoff_sqr)
...
flv terms::evali(const model& m) const {
  ...
  VINA_FOR_IN(i, pairs) {
    ...
    if (d2 > max_r_cutoff_sqr) continue;  // most likely scenario
    fl d = std::sqrt(d2);
    eval_additive_aux(m, atom_index(ip.a, false), atom_index(ip.b, false), d,
                      tmp);
  }
  return tmp;
}
...
flv terms::evale_robust(const model& m) const {
  VINA_CHECK(m.ligands.size() ==
             1);  // only single-ligand systems are supported by this procedure
  ...
  VINA_FOR_IN(j, m.grid_atoms) {
    ...
    if (brick_distance_sqr(box_begin, box_end, a.coords) < max_r_cutoff_sqr &&
        t < n)  // exclude, say, Hydrogens
  ...
  VINA_FOR_IN(j, m.atoms) {
    ...
    const sz t = a.get(m.atom_typing_used());
    if (brick_distance_sqr(box_begin, box_end, a_coords) < max_r_cutoff_sqr &&
        t < n)  // exclude, say, Hydrogens
...
fl terms::eval_conf_independent(
    const conf_independent_inputs& in, fl x,
    flv::const_iterator& it) const {  // evaluates enabled only
  VINA_FOR_IN(i, conf_independent_terms)
