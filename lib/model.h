...
struct interacting_pair {
  ...
  sz a;
  sz b;
  interacting_pair(sz type_pair_index_, sz a_, sz b_)
...
struct ligand : public flexible_body, atom_range {
  unsigned
      degrees_of_freedom;  // can be different from the apparent number of
                           // rotatable bonds, because of the disabled torsions
...
struct pdbqt_initializer;        // forward declaration - only declared in
                                 // parse_pdbqt.cpp
struct model_test;
...
struct model {
  ...
  conf get_initial_conf() const;  // torsions = 0, orientations = identity,
                                  // ligand positions = current
  ...
  void write_ligand(sz ligand_number, const path& name,
  ...
  void write_model(ofile& out, sz model_number,
                   const std::string& remark) const {
 ...
 private:
  ...
  void write_context(const context& c, ofile& out,
                     const std::string& remark) const {
    out << remark;
  }
  ...
  void assign_bonds(const distance_type_matrix&
                        mobility);  // assign bonds based on relative mobility,
                                    // distance and covalent length
  void assign_types();
  ...
  interacting_pairs
      other_pairs;  // all except internal to one ligand: ligand-other ligands;
