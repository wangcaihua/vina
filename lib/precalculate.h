...
struct precalculate_element {
  ...
  fl eval_fast(fl r2) const {
    ...
    sz i = sz(factor * r2);  // r2 is expected < cutoff_sqr, and cutoff_sqr *
                             // factor + 1 < n, so no overflow
    assert(i < fast.size());
    return fast[i];
  }
  pr eval_deriv(fl r2) const {
    ...
    sz i2 = i1 + 1;  // r2 is expected < cutoff_sqr, and cutoff_sqr * factor + 1
                     // < n, so no overflow
  ...
  sz min_smooth_fst() const {
    ...
    VINA_FOR_IN(i_inv, smooth) {
      sz i = smooth.size() - i_inv -
             1;  // i_inv < smooth.size()  => i_inv + 1 <= smooth.size()
      if (i_inv == 0 || smooth[i].first < smooth[tmp].first) tmp = i;
    }
    return tmp;
  }
...
struct precalculate {
  precalculate(const scoring_function& sf, fl v = max_fl, fl factor_ = 32)
      :  // sf should not be discontinuous, even near cutoff, for the sake of
         // the derivatives
        m_cutoff_sqr(sqr(sf.cutoff())),
        n(sz(factor_ * m_cutoff_sqr) +
          3),  // sz(factor * r^2) + 1 <= sz(factor * cutoff_sqr) + 2 <= n-1 < n
               // // see assert below
        factor(factor_),

        data(num_atom_types(sf.atom_typing_used()),
             precalculate_element(n, factor_)),
        m_atom_typing_used(sf.atom_typing_used()) {
    VINA_CHECK(factor > epsilon_fl);
    VINA_CHECK(sz(m_cutoff_sqr * factor) + 1 <
               n);  // cutoff_sqr * factor is the largest float we may end up
                    // converting into sz, then 1 can be added to the result
  ...
  sz index_permissive(sz t1, sz t2) const {
    return data.index_permissive(t1, t2);
  }
