...
struct manifold {
  ...
  fl relative_pair_cost;  // the cost of calculating  pairwise energy /
                          // interaction of atom with grid
  ...
  void print() const {
    ...
    std::cout << "}\n"
              ...
              << ",\nmin_rmsd=" << min_rmsd << ",\nhunt_cap=" << hunt_cap[0]
              << " " << hunt_cap[1] << " " << hunt_cap[2]
              << ",\nexclusion=" << exclusion.position << " "
              << exclusion.orientation << " " << exclusion.torsion
  ...
  manifold()
      : num_phases(800),
        ...
        num_attempts(10),
        num_final_tunings(1),
        /*manifold_mid_factor(0.1), */ manifold_lambda(0),
        cost_factor(0.5),
        max_prob(0.9),
        rstart_fraction(0.25),
        min_rmsd(2),
        // hunt_cap(2, 0.1, 2),
