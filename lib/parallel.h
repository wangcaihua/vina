...
struct parallel_for : private boost::thread_group {
  parallel_for(const F* f, sz num_threads)
      : m_f(f),
        destructing(false),
        size(0),
        thread_finished(num_threads, true),
        count_finished(0),
  ...
  void run(sz size_) {
    ...
    while (count_finished < num_threads)  // wait until processing of all
                                          // elements is thread_finished
      busy.wait(self_lk);
  }
 ...
 private:
  void loop(sz offset) {
    while (boost::optional<sz> sz_option = get_size(offset)) {
      sz s = sz_option.get();
      for (sz i = offset; i < s; i += num_threads) (*m_f)(i);
      {
        boost::mutex::scoped_lock self_lk(self);
  ...
  sz num_threads;
  boost::mutex
      self;  // any modification or reading of mutables should lock this first
  boost::optional<sz> get_size(sz offset) {
    boost::mutex::scoped_lock self_lk(self);
    while (!destructing && thread_finished[offset]) cond.wait(self_lk);
...
struct parallel_for<F, true> : private boost::thread_group {
  parallel_for(const F* f, sz num_threads)
      : m_f(f), destructing(false), size(0), started(0), finished(0) {
  ...
  void run(sz size_) {
    ...
    cond.notify_all();  // many things modified
    while (finished <
           size)  // wait until processing of all elements is finished
      busy.wait(self_lk);
  }
  virtual ~parallel_for() {
    {
 ...
 private:
  ...
  boost::optional<sz> get_next() {
    boost::mutex::scoped_lock self_lk(self);
    while (!destructing && started >= size) cond.wait(self_lk);
