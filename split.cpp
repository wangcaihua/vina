...
path make_path(const std::string& str) { return path(str); }

std::string default_prefix(const std::string& input_name,
...
void write_multimodel_pdbqt(const models& m, const std::string& ligand_prefix,
                            const std::string& flex_prefix) {
  sz how_many = m.size();
...
int main(int argc, char* argv[]) {
  ...
  const std::string version_string =
      "AutoDock Vina PDBQT Split 1.1.2 (May 11, 2011)";
...
Thank you!\n";
  ...
  try {
    ...
    inputs.add_options()("input", value<std::string>(&input_name),
                         "input to split (PDBQT)");
    options_description outputs(
        "Output (optional) - defaults are chosen based on the input file name");
    outputs.add_options()("ligand", value<std::string>(&ligand_prefix),
                          "prefix for ligands")(
        "flex", value<std::string>(&flex_prefix), "prefix for side chains");
    options_description info("Information (optional)");
    info.add_options()("help", bool_switch(&help), "print this message")(
